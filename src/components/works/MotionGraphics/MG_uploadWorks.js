...
export default function MG_uploadWorks(props) {
    ...
    const handleUpload = () => {
        const promises = []
        images.map((image) => {
            const uploadTask = storage.ref(`images/MotionGraphics/${directory}/works/${image.name}`).put(image);
            ...
            uploadTask.on(
                ...
                async () => {
                    await storage
                        .ref(`images/MotionGraphics/${directory}/works/`)
                        .child(image.name)
                        .getDownloadURL()
                        .then((urls) => {
